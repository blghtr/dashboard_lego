...
class TestStaticChartBlock:
    def test_layout(self, datasource_factory):
        ...
        block = StaticChartBlock("test_chart", mock_ds, "My Chart", lambda df: go.Figure(), "state_id")
        layout = block.layout()
    ...
    def test_update_chart_with_data(self, datasource_factory, mock_plot_fn):
        ...
        block = StaticChartBlock("test_chart", mock_ds, "My Chart", mock_plot_fn, "state_id")
...
class TestInteractiveChartBlock:
    ...
    def test_layout(self, datasource_factory, controls):
        mock_ds = datasource_factory()
        block = InteractiveChartBlock("interactive", mock_ds, "Interactive Chart", lambda df: go.Figure(), controls)
        layout = block.layout()
    ...
    def test_update_chart(self, datasource_factory, controls, mock_plot_fn):
        ...
        mock_ds = datasource_factory(get_processed_data=df)
        block = InteractiveChartBlock("interactive", mock_ds, "My Chart", mock_plot_fn, controls)
    ...
    def test_state_registration(self, datasource_factory, controls):
        mock_ds = datasource_factory()
        block = InteractiveChartBlock("interactive", mock_ds, "My Chart", lambda df: go.Figure(), controls)
        state_manager = StateManager()
        block._register_state_interactions(state_manager)

        # Check publishers
        assert "interactive-dropdown" in state_manager.dependency_graph
        assert state_manager.dependency_graph["interactive-dropdown"]['publisher']['component_id'] == "interactive-dropdown"
        assert "interactive-slider" in state_manager.dependency_graph

        # Check subscribers
        # The block subscribes to its own publishers
        assert len(state_manager.dependency_graph["interactive-dropdown"]['subscribers']) == 1
        assert state_manager.dependency_graph["interactive-dropdown"]['subscribers'][0]['callback_fn'] == block._update_chart
