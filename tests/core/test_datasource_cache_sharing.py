...
:relates-to:
 ...
 - implements: "Test suite for cache sharing across DataSource instances"
...
# LLM:METADATA
# :hierarchy: [Testing | Unit Tests | DataSource | CacheSharing | SameCacheDir]
# :relates-to:
...
def test_cache_sharing_with_same_cache_dir():
    ...
    :hierarchy: [Testing | Unit Tests | DataSource | CacheSharing]
    :covers:
     - target: "DataSource._cache_registry"
    ...
    ds1 = DataSource(data_builder=builder1, cache_dir="/tmp/test_cache_shared")
    ds2 = DataSource(data_builder=builder2, cache_dir="/tmp/test_cache_shared")
...
# LLM:METADATA
# :hierarchy: [Testing | Unit Tests | DataSource | CacheSharing | InMemory]
# :relates-to:
...
def test_cache_sharing_in_memory():
    ...
    ds1 = DataSource(data_builder=builder1, cache_dir=None)
    ds2 = DataSource(data_builder=builder2, cache_dir=None)
...
# LLM:METADATA
# :hierarchy: [Testing | Unit Tests | DataSource | CacheSharing | DifferentDirs]
# :relates-to:
...
def test_cache_independent_different_dirs():
    ...
    ds1 = DataSource(data_builder=builder, cache_dir="/tmp/cache1")
    ds2 = DataSource(data_builder=builder, cache_dir="/tmp/cache2")
...
# LLM:METADATA
# :hierarchy: [Testing | Unit Tests | DataSource | CacheSharing | WithTransformFnIntegration]
# :relates-to:
...
def test_cache_sharing_with_transform_fn_integration():
    ...
    original_ds = DataSource(data_builder=builder, cache_dir=None)
