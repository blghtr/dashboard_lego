...
class StateManager:
    ...
    def register_publisher(
        self, state_id: str, component_id: str, component_prop: str
    ):
        """
    ...
    def generate_callbacks(self, app: Any):
        ...
        try:
            ...
            self.logger.info(
                f"Successfully registered {callback_count} callbacks"
            )

        except Exception as e:
            self.logger.error(f"Error generating callbacks: {e}", exc_info=True)
            raise StateError(
                f"Failed to generate callbacks: {e}"
            ) from e
    ...
    def bind_callbacks(self, app: Any, blocks: List[Any]):
        ...
        try:
            ...
            for block in blocks:
                ...
                if not inputs:
                    self.logger.debug(f"Block {block.block_id} has no control inputs, skipping callback")
                    continue
                ...
                def create_block_callback(block_ref):
                    def block_callback(*values):
                        ...
                        control_values = {}
                        for i, (component_id, prop) in enumerate(block_ref.list_control_inputs()):
            ...
            self.logger.info(f"Successfully registered {callback_count} block callbacks")
    ...
    def _validate_no_duplicate_outputs(self, blocks: List[Any]):
        ...
        :relates-to:
         - motivated_by: "ARCHITECTURE_REFACTOR_PLAN Step 7: Validation and tests"
        ...
        self.logger.debug(f"Output validation passed: {len(output_targets)} unique targets")
    ...
    def _create_callback_wrapper(
        ...
        def callback_wrapper(value: Any) -> tuple:
            ...
            try:
                ...
                results = tuple(
                    sub['callback_fn'](value) for sub in subscribers
                )
                self.logger.debug(
                    f"Multi-subscriber callback completed: "
                ...
                self.logger.error(
                    f"Error in callback execution: {e}", exc_info=True
                )
